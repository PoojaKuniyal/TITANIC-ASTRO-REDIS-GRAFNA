...
class DataIngestion:
    ...
    def extract_data(self):
        try:
            conn = self.connect_to_db() # Connects to the DB.

            query = "SELECT * FROM public.titanic" # Runs a SQL query to fetch all rows from the titanic table

            df = pd.read_sql_query(query,conn)  # Converts the result into a Pandas DataFrame.

            conn.close() # Closes the connection

            logger.info("Data extracted from DB")
            return df # returns the data
    ...
    def run(self): # # Orchestrates the pipeline: extract → split → save
        try:
