...
class DataProcessing:
    def __init__(self, train_data_path, test_data_path, feature_store: RedisFeatureStore):
        ...
        self.feature_store = feature_store # will automatically redirects to above feature store
        logger.info("Your Data Processing is initilalized.....")
    ...
    def store_feature_in_redis(self):
        try:
            batch_data = {}
            for idx,row in self.data.iterrows(): # loop into rows
                entity_id = row["PassengerId"] # fetch the entity_id (unique_id)
                features = {
                    "Age" : row['Age'],
                    "Fare": row['Fare'],
                    "Pclass": row["Pclass"],
            ...
            logger.error("Error while feature storing data ")
            raise CustomException(str(e))
    ...
    def run(self):
        try:
            logger.info("Starting Data processing piepline")
            self.load_data()
...
if __name__ =="__main__":
    ...
    print(data_processor.retrieve_feature_redis_store(entity_id=282))